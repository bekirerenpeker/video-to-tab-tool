    ...
    def process_roi(self, roi):
        ...
        try:
            ...
            data = pytesseract.image_to_data(
                roi, config=engine_config, output_type=pytesseract.Output.DICT
            )
...
def debug_and_recognize_characters_threaded(
    frame, all_notes_data, string_y_positions, min_confidence=40
):
    """
        ...
        for x_pos, [x, y, w, h] in all_notes_data[i]:
            ...
            if roi is not None:
                ...
                detected_lines = cv2.morphologyEx(
                    binary_inv, cv2.MORPH_OPEN, string_kernel
                )
                ...
                roi = cv2.resize(
                    roi, None, fx=scaling, fy=scaling, interpolation=cv2.INTER_CUBIC
                )
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=_ocr_pool.pool_size
    ) as executor:
