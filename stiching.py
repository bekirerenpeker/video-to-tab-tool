...
from data_export import import_raw_tab_data, read_list, save_final_tab
...
def cluster_notes_to_tab(all_frames, offsets):
    ...
    for s_idx in range(6):
        ...
        for label in set(labels):
            ...
            for f in raw_frets:
                ...
                if digit_match:
                    processed_symbols.append(digit_match)
                elif "X" in f.upper():
                    processed_symbols.append("X")
                elif "h" in f:
                    processed_symbols.append("h")
                elif "p" in f:
                    processed_symbols.append("p")
                elif "/" in f:
                    ...
                    processed_symbols.append("_")
                elif "v" in f:
                    processed_symbols.append("v")
                elif "^" in f:
                    processed_symbols.append("^")
            ...
            if processed_symbols:
                ...
                if "_" in processed_symbols and (
                    "h" in processed_symbols or "p" in processed_symbols
                ):
                    consensus_val = "_"
                ...
                if is_harmonic and consensus_val != "X":
                    consensus_val = f"<{consensus_val}>"
                if is_ghost and consensus_val != "X":
                    consensus_val = f"({consensus_val})"
            else:
                consensus_val = "N"

            if consensus_val == "N":
                continue  # skip "N" values in the final tab
    ...
    if final_tab:
        ...
        if indices_to_remove:
            final_tab = [
                n for idx, n in enumerate(final_tab) if idx not in indices_to_remove
            ]
    ...
    if os.path.exists(templates_file):
        ...
        if global_templates:
            ...
            for label in set(t_labels):
                ...
                for s_idx in range(6):
                    final_tab.append(
                        {"x": avg_x, "string": s_idx, "fret": consensus_ascii[s_idx]}
                    )
...
def show_stitching_debug(global_points, final_tab):
    ...
    while True:
        ...
        for gx, s_idx, fret, _ in global_points:
            ...
            if dist < hover_radius:
                ...
                if 0 <= screen_x < view_width:
                    ...
                    alpha = max(
                        0.1, 1.0 - (dist / hover_radius)
                    )  # Fade based on distance
                    color = (
                        (150, 150, 150) if dist > 5 else (0, 0, 255)
                    )  # Turn red if directly under mouse
        ...
        cv2.putText(
            canvas,
            "A/D: Scroll | Hover to reveal raw frame data",
            (20, 30),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (100, 100, 100),
        ...
        if key == ord("d"):
            ...
            view_offset = max(0, view_offset - 100)
