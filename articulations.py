...
def is_down_up_stroke(contour, avg_spacing, frame):
    ...
    if h < avg_spacing * 1.5:
        return None
    if w < avg_spacing * 0.5 or w > avg_spacing * 1.2:
        return None
    ...
    if middle_chunk.size > 0:
        mid_cnts, _ = cv2.findContours(
            middle_chunk, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        if mid_cnts:
            m_cnt = max(mid_cnts, key=cv2.contourArea)
            mx, my, mw, mh = cv2.boundingRect(m_cnt)
            if mw < avg_spacing * 0.2:
                is_bar = True
    ...
    if not is_bar:
        return None
    if top_density < bottom_density * 0.6:
        return (contour, "up")
    elif bottom_density < top_density * 0.6:
        return (contour, "down")
...
def detect_and_remove_down_up_strokes(frame, string_y_positions):
    ...
    for s in strokes:
        ...
        contained_strings = [
            i
            for i, s_y in enumerate(string_y_positions)
        ...
        if contained_strings:
            ...
            top_string_idx = min(
                range(len(string_y_positions)),
                key=lambda i: abs(string_y_positions[i] - top),
            )
...
def is_arpeggio(contour, avg_spacing, frame):
    x, y, w, h = cv2.boundingRect(contour)
    if h < avg_spacing * 2.0:
        return False
    if w < avg_spacing * 0.15 or w > avg_spacing * 0.5:
        return False
    ...
    for row in roi:
        ...
        if len(on_pixels) > 0:
            centers.append(np.mean(on_pixels))
    ...
    for i in range(1, len(smoothed_centers)):
        ...
        ) or (smoothed_centers[i - 1] > local_avg and smoothed_centers[i] <= local_avg):
            crossings += 1
...
def detect_and_remove_arp_strokes(frame, string_y_positions):
    ...
    for a in arps:
        ...
        if contained_strings:
            ...
            top_string_idx = min(
                range(len(string_y_positions)),
                key=lambda i: abs(string_y_positions[i] - arp_top),
            )
            bottom_string_idx = top_string_idx
...
def detect_and_remove_vertical_bars(frame, string_y_positions):
    ...
    contours, _ = cv2.findContours(
        vertical_spines, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    for cnt in contours:
        ...
        if w > avg_spacing * 0.12:
            continue
        if abs(y - string_y_positions[0]) > avg_spacing * 0.3:
            continue
        if h < avg_spacing * 4.5:
            continue
...
def get_arch_data(contour, avg_spacing):
    ...
    try:
        ...
        if abs(vertex_x - middle_x) > 0.6 * avg_spacing:
            return None
        orientation = "up" if a < 0 else "down"

        return {"bbox": bbox, "contour": contour, "orientation": orientation}
...
def detect_and_remove_hammer_ons_pull_offs(frame, string_y_positions):
    ...
    for data in arches_data:
        ...
        if dist > (avg_spacing * 1.2):
            continue
        if (
            str_idx == -1
            or abs(string_y_positions[str_idx] - center_y) > avg_spacing * 1.5
        ):
            continue
...
def get_line_data(contour, avg_spacing):
    ...
    if width < avg_spacing * 0.05 or height < avg_spacing * 0.05:
        return None
...
def detect_and_remove_slides(frame, string_y_positions):
    ...
    avg_spacing = abs(string_y_positions[0] - string_y_positions[-1]) / 5
    lines_data = [
        e for e in [get_line_data(c, avg_spacing) for c in contours] if e != None
    ]
