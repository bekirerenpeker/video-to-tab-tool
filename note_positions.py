...
from articulations import (
    detect_and_remove_arp_strokes,
    detect_and_remove_down_up_strokes,
    detect_and_remove_hammer_ons_pull_offs,
    detect_and_remove_slides,
    detect_and_remove_vertical_bars,
)
...
def preprocess_for_numbers(frame, avg_spacing):
    ...
    if is_light_bg:
        processed = cv2.morphologyEx(processed, cv2.MORPH_BLACKHAT, line_kernel_h)
    else:
        processed = cv2.morphologyEx(processed, cv2.MORPH_TOPHAT, line_kernel_h)
    if DEBUG:
        cv2.imwrite(f"{OUTPUT_DIR}/01_numbers_only.png", processed)
...
def detect_shape_bboxes(frame, avg_spacing):
    ...
    for cnt in contours:
        ...
        if w < avg_spacing * 0.3 or h < avg_spacing * 0.4:
            continue
...
def map_shapes_to_strings(bboxes, string_y_positions):
    ...
    for i, (x, y, w, h) in enumerate(bboxes):
        if h < avg_spacing * 1.3:
            continue
        note_count = int(round((h / avg_spacing)))
        split_boxes = [
            (x, y + (i * h // note_count), w, h // note_count)
            for i in range(note_count)
        ]
        bboxes[i] = split_boxes[0]
        for b in split_boxes[1:]:
            bboxes.append(b)
